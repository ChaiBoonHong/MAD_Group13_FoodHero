...
RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE m public.merchants%ROWTYPE; landmark public.campus_landmarks%ROWTYPE;
BEGIN
    ...
    SELECT * INTO landmark FROM public.campus_landmarks
    WHERE campus_id=m.campus_id AND is_active=TRUE
      AND (lower(trim(name))=lower(trim(NEW.pickup_location))
        OR (abs(latitude-NEW.latitude)<0.000001 AND abs(longitude-NEW.longitude)<0.000001))
    ORDER BY CASE WHEN lower(trim(name))=lower(trim(NEW.pickup_location)) THEN 0 ELSE 1 END, name LIMIT 1;
    IF NOT FOUND THEN RAISE EXCEPTION 'Select an active pickup landmark in the merchant campus'; END IF;
    NEW.campus_id:=m.campus_id; NEW.pickup_location:=landmark.name;
    NEW.latitude:=landmark.latitude; NEW.longitude:=landmark.longitude;
    RETURN NEW;
END; $$;
...
DECLARE landmark public.campus_landmarks%ROWTYPE;
BEGIN
    SELECT * INTO landmark FROM public.campus_landmarks
    WHERE campus_id=NEW.campus_id AND is_active=TRUE
      AND (lower(trim(name))=lower(trim(NEW.campus_location))
        OR (abs(latitude-NEW.latitude)<0.000001 AND abs(longitude-NEW.longitude)<0.000001))
    ORDER BY CASE WHEN lower(trim(name))=lower(trim(NEW.campus_location)) THEN 0 ELSE 1 END, name LIMIT 1;
    IF NOT FOUND THEN RAISE EXCEPTION 'Select an active pickup landmark in the selected campus'; END IF;
    NEW.campus_location:=landmark.name; NEW.latitude:=landmark.latitude; NEW.longitude:=landmark.longitude;
    RETURN NEW;
END; $$;
DROP TRIGGER IF EXISTS trg_enforce_merchant_campus_landmark ON public.merchants;
CREATE TRIGGER trg_enforce_merchant_campus_landmark BEFORE INSERT OR UPDATE OF campus_id,campus_location,latitude,longitude
ON public.merchants FOR EACH ROW EXECUTE FUNCTION public.enforce_merchant_campus_landmark();
